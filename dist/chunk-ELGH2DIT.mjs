...
function getAnchors(layout) {
  return layout.filter(
    (l) => l.anchor === true && l.static !== true
  );
}
function getFixedItems(layout, context) {
  const statics = getStatics(layout);
  const anchors = getAnchors(layout);
  const movedItem = context?.movedItemId ? getLayoutItem(layout, context.movedItemId) : void 0;
...
function moveElementAwayFromCollision(layout, collidesWith, itemToMove, isUserAction, compactType, cols) {
  ...
  const preventCollision = collidesWith.static === true || collidesWith.anchor === true && itemToMove.anchor !== true;
  if (isUserAction) {
    isUserAction = false;
    const fakeItem = {
    ...
    if (collisionNorth && compactV) {
      const newY2 = collidesWith.y + collidesWith.h;
      return moveElement(
        layout,
        itemToMove,
        void 0,
...
export { bottom, calcGridCellDimensions, calcGridColWidth, calcGridItemPosition, calcGridItemWHPx, calcWH, calcWHRaw, calcXY, calcXYRaw, clamp, cloneLayout, cloneLayoutItem, collides, correctBounds, getAllCollisions, getAnchors, getFirstCollision, getFixedItems, getLayoutItem, getStatics, modifyLayout, moveElement, moveElementAwayFromCollision, sortLayoutItems, sortLayoutItemsByColRow, sortLayoutItemsByRowCol, validateLayout, withLayoutItem };
